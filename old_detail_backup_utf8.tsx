...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; border: string }> = {
    ...
    CONCLUIDO: { label: 'Disparo Conclu├¡do', color: '#10b981', bg: 'rgba(16,185,129,0.1)', border: 'rgba(16,185,129,0.3)' },
    'CONCLU├ìDO': { label: 'Disparo Conclu├¡do', color: '#10b981', bg: 'rgba(16,185,129,0.1)', border: 'rgba(16,185,129,0.3)' },
};
...
const ClientSubmissionDetail = () => {
    ...
    const handleSaveSender = async () => {
        ...
        try {
            ...
            await addLog("Informa├º├Áes b├ísicas da campanha atualizadas", 'success');
            await load();
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const handleAssignChange = async (employeeName: string) => {
        ...
        try {
            ...
            await addLog(`Respons├ível alterado para: ${employeeName || 'Nenhum'}`, 'info');
            await load();
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const handleNotifyWebhook = async () => {
        ...
        try {
            ...
            pdf.link(60, 230, 90, 10, { url: dashboardUrl }); // Posicionado sobre a ├írea do link
            ...
            const payload = {
                to: notifyTo,
                PDF: hostedUrl,
                mensagem: 'Seu disparo foi concluido com sucesso, veja o rel├ítorio completo em nosso PDF.',
                id: sub.id,
                cnpj: '63140137000161',
                razao_social: 'Plug e Sales Solu├º├Áes digitais LTDA'
            };
            ...
            await addLog("Notifica├º├úo de conclus├úo (PDF) enviada para WhatsApp", 'success');
            alert("Notifica├º├úo enfileirada com sucesso! O sistema processar├í o envio em instantes.");
        } catch (err) {
            console.error("Webhook error:", err);
            alert("Erro ao gerar PDF ou enviar notifica├º├úo.");
        } finally {
        ...
        try {
            ...
            const res = await fetch('/api/shortener/create', {
                ...
                body: JSON.stringify({
                    ...
                    title: `Relat├│rio de Disparo - ${sub.profile_name}`
                })
            });
            const data = await res.json();
            if (data.success && data.shortUrl) {
                copyToClipboard(data.shortUrl, 'Link Encurtador');
                await addLog("Link curto gerado para acesso ao relat├│rio", 'info');
            } else {
    ...
    const handleAddAd = async () => {
        if (!sub) return;
        const newAds = [...(sub.ads || []), {
            ad_name: `Novo An├║ncio ${(sub.ads?.length || 0) + 1}`,
            template_type: 'TEXT',
        ...
        reader.onload = async (evt) => {
            try {
                ...
                const res = await dbService.addReport({
                    ...
                    submissionId: sub.id,
                    reportName: `Relat├│rio: ${sub.profile_name}`,
                    filename: file.name,
                    data: rawData,
                    summary: summary
                });
                ...
                if (res.success) {
                    await load();
                    await addLog(`Relat├│rio de entrega anexado: ${file.name}`, 'success');
                    alert("Ô£à Relat├│rio de entrega anexado com sucesso!");
                } else {
                    alert("ÔØî Erro ao salvar relat├│rio: " + res.error);
                }
            } catch (err) {
                console.error(err);
                alert("ÔØî Erro ao ler o arquivo Excel.");
            } finally {
                setIsUploadingReport(false);
            }
        };
    ...
    const handleDeleteReport = async () => {
        if (!attachedReport || !window.confirm("Deseja realmente excluir o relat├│rio anexado?")) return;
        try {
            const res = await dbService.deleteReport(attachedReport.id);
            if (res.success) {
                setAttachedReport(null);
                await addLog("Relat├│rio de entrega removido", 'error');
                alert("Ô£à Relat├│rio removido com sucesso.");
            }
        } catch (err) {
            console.error(err);
            alert("ÔØî Erro ao excluir relat├│rio.");
        }
    };
    ...
    const handleDownloadNonDelivered = async () => {
        ...
        try {
            ...
            if (nonDelivered.length === 0) {
                alert("N├úo h├í leads n├úo entregues neste relat├│rio.");
                return;
            }
            ...
            await addLog("Relat├│rio de leads n├úo entregues baixado", 'info');
        } catch (err) {
            console.error("Error downloading non-delivered leads:", err);
            alert("Erro ao gerar arquivo de exporta├º├úo.");
        }
    };
    ...
    const handleApproveSubClient = async (subClientId: number) => {
        ...
        if (!password.trim()) return alert("A senha ├® obrigat├│ria para criar o acesso.");

        try {
            const res = await dbService.approveSubClient(subClientId, password);
            if (res.success) {
                await addLog("Novo cliente de indica├º├úo aprovado e conta criada", 'success');
                fetchSubClients();
            } else {
    ...
    if (!sub) return (
        <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '16px' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontWeight: 700 }}>Submiss├úo n├úo encontrada</p>
            ...
            `}</style>            <div style={{ maxWidth: '1440px', margin: '0 auto' }}>
                ...
                <div className="header-content" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px', gap: '24px' }}>
                    ...
                    <div className="header-actions" style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        {(sub.status === 'CONCLUIDO' || sub.status === 'CONCLU├ìDO') && (
                            <>
                                ...
                                <button onClick={() => window.print()} className="action-btn ghost-btn" style={{ padding: '0 20px', height: 44, display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <Printer size={16} /> <span className="hide-mobile">RELAT├ôRIO PDF</span>
                                </button>
                            </>
                        )}
                ...
                <div className="controls-wrapper" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px', marginBottom: '24px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '16px' }}>
                            {STATUS_DISPLAY_KEYS.map((key) => {
                                ...
                                const isActive = sub.status === key || 
                                    ...
                                    (key === 'CONCLUIDO' && sub.status === 'CONCLU├ìDO');
                    ...
                    {sub.status === 'AGUARDANDO_APROVACAO_PAI' && String(sub.parent_id) === String(user?.id) && (
                        <div className="control-card" style={{ animationDelay: '0.15s', borderColor: 'var(--primary-color)', background: 'rgba(172,248,0,0.03)' }}>
                            <label className="field-label" style={{ color: 'var(--primary-color)' }}><Users size={14} /> A├º├úo do Indicado</label>
                    ...
                    {/* CARD 2: CONFIGURA├ç├âO DE TIME */}
                    <div className="control-card" style={{ animationDelay: '0.2s' }}>
                        <label className="field-label"><Smartphone size={14} /> Configura├º├úo de Entrega</label>
                        <div style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            <div>
                                <label style={{ fontSize: '9px', fontWeight: 900, color: 'var(--text-muted)', marginBottom: '8px', display: 'block' }}>N├ÜMERO SENDER (BM)</label>
                            ...
                            <div>
                                ...
                                <textarea className="field-input" value={notes} onChange={e => setNotes(e.target.value)} rows={2} style={{ resize: 'none' }} placeholder="Anota├º├Áes internas..." readOnly={user?.role === 'CLIENT'} />
                            </div>
                            {user?.role !== 'CLIENT' && (
                                <button className="action-btn primary-btn" onClick={handleSaveSender} disabled={isSaving} style={{ width: '100%' }}>
                                    {isSaving ? <RefreshCw className="animate-spin" size={16} /> : <CheckCircle size={16} />}
                                    {isSaving ? 'SALVANDO...' : 'SALVAR ALTERA├ç├òES'}
                                </button>
                            )}
                        </div>
                    </div>

                    {/* CARD 3: ATIVOS MASTER & RESPONS├üVEL */}
                        ...
                        <div style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            <div style={{ padding: '12px 16px', background: 'var(--card-bg-subtle)', borderRadius: '14px', border: '1px solid var(--surface-border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    ...
                                    <span style={{ fontSize: '11px', fontWeight: 800, color: 'var(--text-muted)' }}>Respons├ível:</span>
                                </div>
                                {user?.role === 'ADMIN' ? (
                                    <select
                                    ...
                                    >
                                        <option value="" style={{ background: 'var(--bg-primary)', color: 'var(--text-muted)' }}>N├úo Atribu├¡do</option>
                                        {employees.map(emp => (
                            ...
                            {attachedReport && (
                                <div className="asset-link" style={{ width: '100%', border: '1px solid var(--primary-color)', cursor: 'default' }}>
                                    ...
                                    >
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <p style={{ margin: 0, fontSize: '12px', fontWeight: 900 }}>RELAT├ôRIO DE ENTREGA</p>
                            ...
                            {(sub.status === 'CONCLUIDO' || sub.status === 'CONCLU├ìDO') && user?.role !== 'CLIENT' && !attachedReport && (
                                <label style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px', background: 'rgba(172,248,0,0.05)', border: '1px dashed var(--primary-color)', borderRadius: '18px', cursor: 'pointer', transition: 'all 0.2s' }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <p style={{ margin: 0, fontSize: '11px', fontWeight: 900, color: 'var(--primary-color)' }}>{isUploadingReport ? 'PROCESSANDO...' : 'ANEXAR RELAT├ôRIO EXCEL'}</p>
                            ...
                            {sub.media_url && (
                                <a href={sub.media_url} target="_blank" rel="noreferrer" className="asset-link">
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <p style={{ margin: 0, fontSize: '12px', fontWeight: 900 }}>M├ìDIA PRINCIPAL</p>
                            ...
                            {(sub.status === 'CONCLUIDO' || sub.status === 'CONCLU├ìDO') && user?.role !== 'CLIENT' && (
                                <button
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                    {(!sub.logs || sub.logs.length === 0) ? (
                                        <div style={{ padding: '16px', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', border: '1px dashed var(--surface-border-subtle)', textAlign: 'center' }}>
                                            <p style={{ margin: 0, fontSize: '10px', opacity: 0.4 }}>Nenhum hist├│rico registrado.</p>
                                        </div>
                                    ) : (
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                                            ...
                                            <div>
                                                ...
                                                <span style={{ fontSize: '10px', fontWeight: 700, opacity: 0.5, textTransform: 'uppercase' }}>
                                                    {sc.approved ? 'CADASTRO APROVADO' : 'AGUARDANDO APROVA├ç├âO'}
                                                </span>
                                            </div>
                                        </div>
                ...
                {/* ÔöÇÔöÇ ADS ANALYZER ÔöÇÔöÇ */}
                <div className="control-card" style={{ animationDelay: '0.4s' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', gap: '16px', flexWrap: 'wrap' }}>
                        <label className="field-label" style={{ marginBottom: 0 }}><Layers size={14} /> Navegador de An├║ncios ({(sub.ads || []).length})</label>
                    ...
                    {currentAd ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                            {/* M├ëTRICAS E FATURAMENTO DO AD (Substitu├¡do por Condi├º├úo ├Ünica) */}
                            {(sub.status === 'GERADO' || sub.status === 'CONCLUIDO') && (
                                <div style={{ background: 'rgba(255,255,255,0.015)', padding: '24px', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px' }}>
                                    ...
                                    {/* Relat├│rio Final Vis├¡vel para Admin + Client */}
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                                        ...
                                        {user?.role === 'ADMIN' && (
                                            <div className="no-print" style={{ background: 'var(--card-bg-subtle)', padding: '20px', borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '8px', border: '1px solid rgba(239,68,68,0.2)' }}>
                                                ...
                                                <span style={{ fontSize: '10px', fontWeight: 900, color: '#ef4444', letterSpacing: '1px' }}>CUSTO AG├èNCIA</span>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', minWidth: 0 }}>
                                    <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.06)', padding: '24px', height: '100%', display: 'flex', flexDirection: 'column' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px', flexShrink: 0 }}>
                                            <span className="no-print" style={{ fontSize: '10px', fontWeight: 900, color: 'var(--primary-color)', letterSpacing: '2px' }}>CONTE├ÜDO DA MENSAGEM</span>
                                        ...
                                        <div className="no-print" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                                            {user?.role !== 'CLIENT' ? (
                                                <textarea
                                                    ...
                                                    placeholder="Digite o texto do an├║ncio aqui..."
                                                />
                                            ) : (
                                                <div style={{
                                                    padding: '16px',
                                        ...
                                        <div className="print-only" style={{ fontSize: '16px', lineHeight: '1.8', color: '#000', fontWeight: 800, padding: '16px', border: '2px dashed #ccc', borderRadius: '12px', marginTop: '10px' }}>
                                            ...
                                            Plug e Sales Solu├º├Áes digitais LTDA
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', minWidth: 0 }}>
                                        ...
                                        <div style={{ padding: '16px', background: 'var(--card-bg-subtle)', borderRadius: '16px', border: '1px solid var(--surface-border-subtle)' }}>
                                            ...
                                            {user?.role !== 'CLIENT' ? (
                                                ...
                                                >
                                                    ...
                                                    <option value="VIDEO">V├ìDEO</option>
                                                    <option value="DOCUMENT">DOCUMENTO</option>
                                                    <option value="LOCATION">LOCALIZA├ç├âO</option>
                                                </select>
                                            ) : (
                                    ...
                                    {user?.role !== 'CLIENT' && (['IMAGE', 'VIDEO', 'DOCUMENT'].includes(currentAd.template_type || '')) && (
                                        <div style={{ padding: '16px', background: 'rgba(255,170,0,0.05)', borderRadius: '16px', border: '1px solid rgba(255,170,0,0.1)' }}>
                                            <label style={{ fontSize: '9px', fontWeight: 900, color: '#ffaa00', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                <Upload size={12} /> UPLOAD DE M├ìDIA ({currentAd.template_type})
                                            </label>
                                    ...
                                    <div style={{ padding: '16px', background: 'var(--card-bg-subtle)', borderRadius: '16px', border: '1px solid var(--surface-border-subtle)', borderLeft: '4px solid #3b82f6' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                            <label style={{ fontSize: '9px', fontWeight: 900, color: '#3b82f6', opacity: 0.8 }}>LINK DO BOT├âO</label>
                                    ...
                                    <div style={{ padding: '16px', background: 'var(--card-bg-subtle)', borderRadius: '16px', border: '1px solid var(--surface-border-subtle)', borderLeft: '4px solid var(--primary-color)' }}>
                                        <label style={{ fontSize: '9px', fontWeight: 900, color: 'var(--primary-color)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px', textTransform: 'uppercase' }}>
                                            <Smartphone size={12} /> N├ÜMERO SENDER (Deste An├║ncio)
                                        </label>
                                        <div style={{ position: 'relative' }}>
                                            <input
                                                className="field-input"
                                    ...
                                    <div style={{ padding: '16px', background: 'var(--card-bg-subtle)', borderRadius: '20px', border: '1px solid var(--surface-border-subtle)' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                                            <label className="field-label" style={{ marginBottom: 0 }}>Vari├íveis ({currentAd.variables?.length || 0})</label>
                                            {user?.role !== 'CLIENT' && (
                                                <button
                                                    onClick={() => {
                                                        const name = window.prompt("Nome da nova vari├ível (ex: NOME, VALOR):");
                                        ...
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                                            {currentAd.variables?.map((v, i) => (
                                                <div key={i} style={{ 
                                                    ...
                                                    <div style={{ display: 'flex', gap: '4px' }}>
                                                        <button
                                                            onClick={() => copyToClipboard(v, `Vari├ível ${v}`)}
                                            ...
                                            {(!currentAd.variables || currentAd.variables.length === 0) && (
                                                <p style={{ margin: 0, fontSize: '11px', opacity: 0.3, padding: '10px', color: 'var(--text-muted)' }}>Nenhuma vari├ível definida para este AD.</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div style={{ padding: '60px', textAlign: 'center', opacity: 0.2 }}>
                            ...
                            <p style={{ fontWeight: 800 }}>Selecione um an├║ncio para visualizar</p>
                        </div>
                    )}
                </div>
                ...
                {copyFeedback && (
                    <div style={{ position: 'fixed', bottom: '32px', left: '50%', transform: 'translateX(-50%)', background: 'var(--primary-color)', color: '#000', padding: '12px 24px', borderRadius: '16px', fontWeight: 900, fontSize: '13px', boxShadow: '0 10px 40px rgba(172,248,0,0.3)', zIndex: 9999, animation: 'fadeInUp 0.3s ease-out' }}>
                        Ô£ô {copyFeedback.toUpperCase()} COPIADO COM SUCESSO!
                    </div>
                )}
            </div>
            ...
            <div style={{ position: 'absolute', left: '-9999px', top: 0 }}>
                <div ref={reportRef} style={{ width: '800px', padding: '60px', background: '#fff', color: '#000', fontFamily: 'Arial, sans-serif' }}>
                    <div style={{ borderBottom: '2px solid #000', paddingBottom: '20px', marginBottom: '30px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                        <div>
                            <h1 style={{ margin: 0, fontSize: '32px', fontWeight: 900, color: '#000' }}>RELAT├ôRIO DE DISPARO</h1>
                            <p style={{ margin: '8px 0 0 0', fontSize: '16px', fontWeight: 700, opacity: 0.6 }}>
                                Envio Profissional #{sub?.id}
                            </p>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                            <p style={{ margin: 0, fontSize: '14px', fontWeight: 900, color: '#10b981' }}>STATUS: DISPARO CONCLU├ìDO</p>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '40px' }}>
                        <div style={{ padding: '25px', background: '#fff', borderRadius: '15px', border: '1px solid #e2e8f0' }}>
                            <h3 style={{ fontSize: '12px', color: '#64748b', marginBottom: '15px', textTransform: 'uppercase', fontWeight: 900 }}>Anal├¡tico de Convers├úo</h3>
                    ...
                    <div style={{ background: '#f8fafc', padding: '25px', borderRadius: '20px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '30px' }}>
                        ...
                        <div style={{ flex: 1 }}>
                            <h3 style={{ fontSize: '10px', color: '#64748b', margin: '0 0 5px 0', textTransform: 'uppercase', fontWeight: 900 }}>Visualiza├º├úo Exclusiva</h3>
                    ...
                    <div style={{ marginTop: 'auto', paddingTop: '40px', borderTop: '2px solid #000', textAlign: 'center' }}>
                        ...
                        <p style={{ margin: '8px 0 0 0', fontSize: '18px', fontWeight: 900 }}>Plug e Sales Solu├º├Áes digitais LTDA</p>
                        <div style={{ marginTop: '30px', display: 'flex', justifyContent: 'center', gap: '20px', fontSize: '10px', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '1px' }}>
                            <span>Relat├│rio Oficial de Entrega</span>
                            <span>ÔÇó</span>
